...
void Grid::init(float levelWidth, float levelHeight, PragmaEngine::ParticleBatch2D* bloodParticleBatch) {
...
void Grid::draw(PragmaEngine::SpriteBatch& spriteBatch, PragmaEngine::Camera2D& camera) {
...
Cell& Grid::getContainingCell(const glm::vec2& pos) {
	...
	// Clamp position
	if (cellX < 0) { cellX = 0; }
	else if (cellX >= m_cells[cellY].size()) { cellX = m_cells[cellY].size() - 1; }
	if (cellY < 0) { cellY = 0; }
	else if (cellY >= m_cells.size()) { cellY = m_cells.size() - 1; }
	// Get the cell
...
void Grid::addBlood(const glm::vec2& position, int numParticles) {
	...
	PragmaEngine::ColorRGBA8 col(255, 0, 0, 255);
