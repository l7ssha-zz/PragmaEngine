...
void Player::init(b2World* world, const glm::vec2& position, const glm::vec2& dimensions, PragmaEngine::ColorRGBA8 color) {
    PragmaEngine::GLTexture texture = PragmaEngine::ResourceManager::getTexture("Assets/blue_ninja.png");
    m_collisionBox.init(world, position, dimensions, texture, color, true, glm::vec4(0.0f, 0.0f, 0.1f, 0.5f));
}

void Player::draw(PragmaEngine::SpriteBatch& spriteBatch) {
    m_collisionBox.draw(spriteBatch);
}

void Player::update(PragmaEngine::InputManager& inputManager) {
